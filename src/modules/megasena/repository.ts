...
export class MegasenaRepository implements GameRepository {
  async count(): Promise<number> {
    return prisma.megasenaDraw.count();
  }

  async getLatestContest(): Promise<number | null> {
    const latest = await prisma.megasenaDraw.findFirst({
  ...
  async contestExists(contestNumber: number): Promise<boolean> {
    const row = await prisma.megasenaDraw.findUnique({
      where: { contestNumber },
      select: { contestNumber: true },
    });
    return row !== null;
  ...
  async findMany(filter?: DrawFilter): Promise<DrawRecord[]> {
    ...
    const draws = await prisma.megasenaDraw.findMany({
      where,
      orderBy: { contestNumber: "desc" },
      take: filter?.limit,
    });
  ...
  async upsert(draw: NormalizedDraw): Promise<void> {
    ...
    await prisma.megasenaDraw.upsert({
